/*******************************************************************************
* File Name: NEOMOTE_1_UART_MOTE_IntClock.c
* Version 2.0
*
*  Description:
...
/*******************************************************************************
* Function Name: NEOMOTE_1_UART_MOTE_IntClock_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void NEOMOTE_1_UART_MOTE_IntClock_Start(void) 
{
    /* Set the bit to enable the clock. */
    NEOMOTE_1_UART_MOTE_IntClock_CLKEN |= NEOMOTE_1_UART_MOTE_IntClock_CLKEN_MASK;
	NEOMOTE_1_UART_MOTE_IntClock_CLKSTBY |= NEOMOTE_1_UART_MOTE_IntClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: NEOMOTE_1_UART_MOTE_IntClock_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void NEOMOTE_1_UART_MOTE_IntClock_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    NEOMOTE_1_UART_MOTE_IntClock_CLKEN &= (uint8)(~NEOMOTE_1_UART_MOTE_IntClock_CLKEN_MASK);
	NEOMOTE_1_UART_MOTE_IntClock_CLKSTBY &= (uint8)(~NEOMOTE_1_UART_MOTE_IntClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: NEOMOTE_1_UART_MOTE_IntClock_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void NEOMOTE_1_UART_MOTE_IntClock_StopBlock(void) 
{
    if ((NEOMOTE_1_UART_MOTE_IntClock_CLKEN & NEOMOTE_1_UART_MOTE_IntClock_CLKEN_MASK) != 0u)
    {
...
#if defined(NEOMOTE_1_UART_MOTE_IntClock__CFG3)
        CLK_DIST_AMASK = NEOMOTE_1_UART_MOTE_IntClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = NEOMOTE_1_UART_MOTE_IntClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* NEOMOTE_1_UART_MOTE_IntClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(NEOMOTE_1_UART_MOTE_IntClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(NEOMOTE_1_UART_MOTE_IntClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: NEOMOTE_1_UART_MOTE_IntClock_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void NEOMOTE_1_UART_MOTE_IntClock_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        NEOMOTE_1_UART_MOTE_IntClock_CLKSTBY &= (uint8)(~NEOMOTE_1_UART_MOTE_IntClock_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: NEOMOTE_1_UART_MOTE_IntClock_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void NEOMOTE_1_UART_MOTE_IntClock_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = NEOMOTE_1_UART_MOTE_IntClock_GetSourceRegister();
    uint16 oldDivider = NEOMOTE_1_UART_MOTE_IntClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = NEOMOTE_1_UART_MOTE_IntClock_CLKEN & NEOMOTE_1_UART_MOTE_IntClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(NEOMOTE_1_UART_MOTE_IntClock_DIV_PTR, clkDivider);
                NEOMOTE_1_UART_MOTE_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                NEOMOTE_1_UART_MOTE_IntClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(NEOMOTE_1_UART_MOTE_IntClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((NEOMOTE_1_UART_MOTE_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(NEOMOTE_1_UART_MOTE_IntClock_DIV_PTR, clkDivider);
				NEOMOTE_1_UART_MOTE_IntClock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: NEOMOTE_1_UART_MOTE_IntClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 NEOMOTE_1_UART_MOTE_IntClock_GetDividerRegister(void) 
{
    return CY_GET_REG16(NEOMOTE_1_UART_MOTE_IntClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: NEOMOTE_1_UART_MOTE_IntClock_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void NEOMOTE_1_UART_MOTE_IntClock_SetModeRegister(uint8 modeBitMask) 
{
    NEOMOTE_1_UART_MOTE_IntClock_MOD_SRC |= modeBitMask & (uint8)NEOMOTE_1_UART_MOTE_IntClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: NEOMOTE_1_UART_MOTE_IntClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void NEOMOTE_1_UART_MOTE_IntClock_ClearModeRegister(uint8 modeBitMask) 
{
    NEOMOTE_1_UART_MOTE_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(NEOMOTE_1_UART_MOTE_IntClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: NEOMOTE_1_UART_MOTE_IntClock_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 NEOMOTE_1_UART_MOTE_IntClock_GetModeRegister(void) 
{
    return NEOMOTE_1_UART_MOTE_IntClock_MOD_SRC & (uint8)(NEOMOTE_1_UART_MOTE_IntClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: NEOMOTE_1_UART_MOTE_IntClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void NEOMOTE_1_UART_MOTE_IntClock_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = NEOMOTE_1_UART_MOTE_IntClock_GetDividerRegister();
    uint8 oldSrc = NEOMOTE_1_UART_MOTE_IntClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        NEOMOTE_1_UART_MOTE_IntClock_MOD_SRC |= CYCLK_SSS;
        NEOMOTE_1_UART_MOTE_IntClock_MOD_SRC =
            (NEOMOTE_1_UART_MOTE_IntClock_MOD_SRC & (uint8)(~NEOMOTE_1_UART_MOTE_IntClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: NEOMOTE_1_UART_MOTE_IntClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 NEOMOTE_1_UART_MOTE_IntClock_GetSourceRegister(void) 
{
    return NEOMOTE_1_UART_MOTE_IntClock_MOD_SRC & NEOMOTE_1_UART_MOTE_IntClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: NEOMOTE_1_UART_MOTE_IntClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void NEOMOTE_1_UART_MOTE_IntClock_SetPhaseRegister(uint8 clkPhase) 
{
    NEOMOTE_1_UART_MOTE_IntClock_PHASE = clkPhase & NEOMOTE_1_UART_MOTE_IntClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: NEOMOTE_1_UART_MOTE_IntClock_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 NEOMOTE_1_UART_MOTE_IntClock_GetPhaseRegister(void) 
{
    return NEOMOTE_1_UART_MOTE_IntClock_PHASE & NEOMOTE_1_UART_MOTE_IntClock_PHASE_MASK;
}
