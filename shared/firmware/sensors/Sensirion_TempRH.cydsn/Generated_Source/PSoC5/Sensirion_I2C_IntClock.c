/*******************************************************************************
* File Name: Sensirion_I2C_IntClock.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: Sensirion_I2C_IntClock_Start
********************************************************************************
*
* Summary:
...
void Sensirion_I2C_IntClock_Start(void) 
{
    ...
    Sensirion_I2C_IntClock_CLKEN |= Sensirion_I2C_IntClock_CLKEN_MASK;
	Sensirion_I2C_IntClock_CLKSTBY |= Sensirion_I2C_IntClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Sensirion_I2C_IntClock_Stop
********************************************************************************
*
* Summary:
...
void Sensirion_I2C_IntClock_Stop(void) 
{
    ...
    Sensirion_I2C_IntClock_CLKEN &= (uint8)(~Sensirion_I2C_IntClock_CLKEN_MASK);
	Sensirion_I2C_IntClock_CLKSTBY &= (uint8)(~Sensirion_I2C_IntClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Sensirion_I2C_IntClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Sensirion_I2C_IntClock_CLKEN & Sensirion_I2C_IntClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Sensirion_I2C_IntClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Sensirion_I2C_IntClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Sensirion_I2C_IntClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Sensirion_I2C_IntClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Sensirion_I2C_IntClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Sensirion_I2C_IntClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Sensirion_I2C_IntClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Sensirion_I2C_IntClock_GetSourceRegister();
    uint16 oldDivider = Sensirion_I2C_IntClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Sensirion_I2C_IntClock_CLKEN & Sensirion_I2C_IntClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Sensirion_I2C_IntClock_DIV_PTR, clkDivider);
                Sensirion_I2C_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Sensirion_I2C_IntClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Sensirion_I2C_IntClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Sensirion_I2C_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(Sensirion_I2C_IntClock_DIV_PTR, clkDivider);
				Sensirion_I2C_IntClock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: Sensirion_I2C_IntClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Sensirion_I2C_IntClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: Sensirion_I2C_IntClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Sensirion_I2C_IntClock_MOD_SRC |= modeBitMask & (uint8)Sensirion_I2C_IntClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: Sensirion_I2C_IntClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Sensirion_I2C_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Sensirion_I2C_IntClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: Sensirion_I2C_IntClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Sensirion_I2C_IntClock_MOD_SRC & (uint8)(Sensirion_I2C_IntClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: Sensirion_I2C_IntClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Sensirion_I2C_IntClock_GetDividerRegister();
    uint8 oldSrc = Sensirion_I2C_IntClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Sensirion_I2C_IntClock_MOD_SRC |= CYCLK_SSS;
        Sensirion_I2C_IntClock_MOD_SRC =
            (Sensirion_I2C_IntClock_MOD_SRC & (uint8)(~Sensirion_I2C_IntClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Sensirion_I2C_IntClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Sensirion_I2C_IntClock_MOD_SRC & Sensirion_I2C_IntClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Sensirion_I2C_IntClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Sensirion_I2C_IntClock_PHASE = clkPhase & Sensirion_I2C_IntClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Sensirion_I2C_IntClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Sensirion_I2C_IntClock_PHASE & Sensirion_I2C_IntClock_PHASE_MASK;
}
