/*******************************************************************************
* File Name: uart_ultrasonic_IntClock.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: uart_ultrasonic_IntClock_Start
********************************************************************************
*
* Summary:
...
void uart_ultrasonic_IntClock_Start(void) 
{
    ...
    uart_ultrasonic_IntClock_CLKEN |= uart_ultrasonic_IntClock_CLKEN_MASK;
	uart_ultrasonic_IntClock_CLKSTBY |= uart_ultrasonic_IntClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: uart_ultrasonic_IntClock_Stop
********************************************************************************
*
* Summary:
...
void uart_ultrasonic_IntClock_Stop(void) 
{
    ...
    uart_ultrasonic_IntClock_CLKEN &= (uint8)(~uart_ultrasonic_IntClock_CLKEN_MASK);
	uart_ultrasonic_IntClock_CLKSTBY &= (uint8)(~uart_ultrasonic_IntClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: uart_ultrasonic_IntClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((uart_ultrasonic_IntClock_CLKEN & uart_ultrasonic_IntClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = uart_ultrasonic_IntClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = uart_ultrasonic_IntClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* uart_ultrasonic_IntClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(uart_ultrasonic_IntClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(uart_ultrasonic_IntClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: uart_ultrasonic_IntClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: uart_ultrasonic_IntClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = uart_ultrasonic_IntClock_GetSourceRegister();
    uint16 oldDivider = uart_ultrasonic_IntClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = uart_ultrasonic_IntClock_CLKEN & uart_ultrasonic_IntClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(uart_ultrasonic_IntClock_DIV_PTR, clkDivider);
                uart_ultrasonic_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                uart_ultrasonic_IntClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(uart_ultrasonic_IntClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((uart_ultrasonic_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(uart_ultrasonic_IntClock_DIV_PTR, clkDivider);
				uart_ultrasonic_IntClock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: uart_ultrasonic_IntClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(uart_ultrasonic_IntClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: uart_ultrasonic_IntClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    uart_ultrasonic_IntClock_MOD_SRC |= modeBitMask & (uint8)uart_ultrasonic_IntClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: uart_ultrasonic_IntClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    uart_ultrasonic_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(uart_ultrasonic_IntClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: uart_ultrasonic_IntClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return uart_ultrasonic_IntClock_MOD_SRC & (uint8)(uart_ultrasonic_IntClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: uart_ultrasonic_IntClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = uart_ultrasonic_IntClock_GetDividerRegister();
    uint8 oldSrc = uart_ultrasonic_IntClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        uart_ultrasonic_IntClock_MOD_SRC |= CYCLK_SSS;
        uart_ultrasonic_IntClock_MOD_SRC =
            (uart_ultrasonic_IntClock_MOD_SRC & (uint8)(~uart_ultrasonic_IntClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: uart_ultrasonic_IntClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return uart_ultrasonic_IntClock_MOD_SRC & uart_ultrasonic_IntClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: uart_ultrasonic_IntClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    uart_ultrasonic_IntClock_PHASE = clkPhase & uart_ultrasonic_IntClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: uart_ultrasonic_IntClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return uart_ultrasonic_IntClock_PHASE & uart_ultrasonic_IntClock_PHASE_MASK;
}
