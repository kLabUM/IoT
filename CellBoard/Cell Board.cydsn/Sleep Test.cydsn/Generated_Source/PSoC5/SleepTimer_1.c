...
*
* Global variables:
*  The SleepTimer_1_initVar variable is used to indicate initial
*  configuration of this component.  The variable is initialized to zero and
*  set to 1 the first time SleepTimer_1_Start() is called. This allows for
*  component initialization without re-initialization in all subsequent calls
*  to the SleepTimer_1_Start() routine.
*
* Reentrant:
*  No
*
...
void SleepTimer_1_Start(void) 
{
    ...
    if (0u == SleepTimer_1_initVar)
    {
        SleepTimer_1_Init();
        SleepTimer_1_initVar = 1u;
    }
...
/*******************************************************************************
* Function Name: SleepTimer_1_Init
********************************************************************************
*
* Summary:
...
void SleepTimer_1_Init(void)  
{
    ...
    SleepTimer_1_SetInterval(SleepTimer_1_INTERVAL);

    /* Check if user sets to issue an interrupt in customizer */
    #if (1u == SleepTimer_1_ENABLE_INTERRUPT)
    {
        /* Enable interrupt */
    ...
    #endif  /* (1u == SleepTimer_1_ENABLE_INTERRUPT) */
}


/*******************************************************************************
* Function Name: SleepTimer_1_Enable
********************************************************************************
* Summary:
...
void SleepTimer_1_Enable(void) 
{
    ...
    if(0u == (SleepTimer_1_ILO_CFG_REG & SleepTimer_1_ILO_1KHZ_EN))
    {
        /* Enter critical section */
        interruptState = CyEnterCriticalSection();

        /* Enable 1kHz ILO */
        SleepTimer_1_ILO_CFG_REG |=SleepTimer_1_ILO_1KHZ_EN;

        /* Enable the CTW counter */
        SleepTimer_1_TW_CFG_REG |= SleepTimer_1_CTW_EN;
...
void SleepTimer_1_Stop(void) 
{
    ...
    /* Disable CTW counter */
    SleepTimer_1_TW_CFG_REG &= (uint8)(~SleepTimer_1_CTW_EN);
...
void SleepTimer_1_EnableInt(void) 
{
    ...
    SleepTimer_1_TW_CFG_REG |= SleepTimer_1_CTW_IE;
...
void SleepTimer_1_DisableInt(void) 
{
    ...
    SleepTimer_1_TW_CFG_REG &= (uint8)(~SleepTimer_1_CTW_IE);
...
*
*           Name                    Value        Period
*   SleepTimer_1__CTW_2_MS      4'b0001        2 ms
*   SleepTimer_1__CTW_4_MS      4'b0010        4 ms
*   SleepTimer_1__CTW_8_MS      4'b0011        8 ms
*   SleepTimer_1__CTW_16_MS     4'b0100        16 ms
*   SleepTimer_1__CTW_32_MS     4'b0101        32 ms
*   SleepTimer_1__CTW_64_MS     4'b0110        64 ms
*   SleepTimer_1__CTW_128_MS    4'b0111        128 ms
*   SleepTimer_1__CTW_256_MS    4'b1000        256 ms
*   SleepTimer_1__CTW_512_MS    4'b1001        512 ms
*   SleepTimer_1__CTW_1024_MS   4'b1010        1024 ms
*   SleepTimer_1__CTW_2048_MS   4'b1011        2048 ms
*   SleepTimer_1__CTW_4096_MS   4'b1100        4096 ms
*
* Return:
*  None
*
...
void SleepTimer_1_SetInterval(uint8 interval) 
{
    ...
    if (0u == (SleepTimer_1_TW_CFG_REG & SleepTimer_1_CTW_EN))
    {
        #if(CY_PSOC5A)

            CYASSERT((SleepTimer_1__CTW_4_MS   == interval) ||
                     (SleepTimer_1__CTW_8_MS   == interval) ||
                     (SleepTimer_1__CTW_16_MS  == interval) ||
                     (SleepTimer_1__CTW_32_MS  == interval) ||
                     (SleepTimer_1__CTW_64_MS  == interval) ||
                     (SleepTimer_1__CTW_128_MS == interval) ||
                     (SleepTimer_1__CTW_256_MS == interval));

        #endif  /* (CY_PSOC5A) */

        /* Set CTW interval */
        SleepTimer_1_CTW_INTERVAL_REG = \
            (SleepTimer_1_CTW_INTERVAL_REG & ((uint8)~SleepTimer_1_INTERVAL_MASK)) | \
            (interval & SleepTimer_1_INTERVAL_MASK);
    }
...
/*******************************************************************************
* Function Name: SleepTimer_1_GetStatus
********************************************************************************
*
* Summary:
...
*
*            Name                             Description
*  SleepTimer_1_PM_INT_SR_ONEPPSP  A onepps event has occured
*  SleepTimer_1_PM_INT_SR_CTW      A central timewheel event occured
*  SleepTimer_1_PM_INT_SR_FTW      A fast timewheel event occured
*
* Side Effects:
